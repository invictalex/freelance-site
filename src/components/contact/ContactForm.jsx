...
export default function ContactForm(){
    ...
    return(
        ...
        <div 
            className="fixed right-20 bottom-60 w-16 h-16 bg-yellow rounded-full flex items-center justify-center z-20"
        >
            <img src="src/images/message.svg" className="w-[100%] m-2 cursor-pointer" 
                onClick={() => setOpen((prevState) => !prevState)}/>


            {open &&
                <div className="fixed bg-[#0009] h-screen w-screen top-0 left-0 flex items-center justify-center text-md">
                    <div className="w-110 bg-white flex justify-center px-5 py-8 rounded-lg border-blue-dk border relative ">
                        <img 
                            src={close} 
                            className="absolute right-3 top-3 w-3 opacity-30 cursor-pointer" 
                            onClick={() => setOpen((prevState) => !prevState)}
                        />
                        <form className="flex flex-col items-start justify-start w-full space-y-5 ">
                            ...
                            <button className="w-full py-1 px-5 bg-slate text-white font-semibold rounded-lg h-10 mt-20
